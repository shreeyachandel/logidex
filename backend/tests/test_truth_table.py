import unittest
from backend.logic.truth_table import generate_truth_table, compute_gate_output, evaluate_output
...
class TestTruthTable(unittest.TestCase):
    ...
    def test_cyclic_circuit_failure(self):
        ...
        try:
            ...
            has_error_output = any('?' in str(row["row"][-1]) for row in result["table"])
            self.assertTrue(has_error_output, "Circuit with cycle should show evaluation errors")
    ...
    def test_missing_inputs_failure(self):
        ...
        try:
            ...
            has_error_output = any('?' in str(row["row"][-1]) for row in result["table"])
            self.assertTrue(has_error_output, "Circuit with missing inputs should show evaluation errors")
        except Exception as e:
