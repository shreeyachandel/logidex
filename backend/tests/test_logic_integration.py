...
class TestLogicIntegration(unittest.TestCase):
    ...
    def test_complex_integration_with_all_gate_types(self):
        ...
        circuit["current_input_combination"] = [
            {"nodeId": node_id, "value": [value]} for node_id, value in input_values.items()
        ]
        ...
        highlighted_index = [i for i, row in enumerate(truth_table["table"]) 
                           if row.get("highlighted") and row["row"][:-1] == all_ones_input]
        
        if highlighted_index:  # If we found the all-1's row
            self.assertEqual(highlighted[0]["row"][-1], 1, "Expected output for all 1's input should be 1")
    ...
    def test_error_recovery_circuit_with_missing_connections(self):
        ...
        has_error_indicator = any('?' in str(row["row"][-1]) for row in truth_table["table"])
        self.assertTrue(has_error_indicator, "Truth table should indicate errors for broken circuit")
