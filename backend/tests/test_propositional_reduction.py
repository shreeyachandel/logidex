import unittest
from logic.propositional_reduction import (
    Formula, Literal, Not, And, Or, True_, False_,
    build_formula, build_xor, build_xnor,
    apply_de_morgans_step, simplify_step,
    distribute_or_over_and_step, distribute_and_over_or_step,
    is_in_cnf, is_in_dnf,
    to_cnf, to_dnf,
...
class TestCircuitLogic(unittest.TestCase):
    ...
    def test_is_in_cnf(self):
        ...
        self.assertTrue(is_in_cnf(And(Or(Literal("A"), Literal("B")), Or(Literal("C"), Literal("D")))))
        
        # Not in CNF
        self.assertFalse(is_in_cnf(Or(And(Literal("A"), Literal("B")), Literal("C"))))
        self.assertFalse(is_in_cnf(And(Or(Literal("A"), And(Literal("B"), Literal("C"))), Literal("D"))))
    ...
    def test_is_in_dnf(self):
        ...
        self.assertTrue(is_in_dnf(Or(And(Literal("A"), Literal("B")), And(Literal("C"), Literal("D")))))
        
        # Not in DNF
        self.assertFalse(is_in_dnf(And(Or(Literal("A"), Literal("B")), Literal("C"))))
        self.assertFalse(is_in_dnf(Or(And(Literal("A"), Or(Literal("B"), Literal("C"))), Literal("D"))))
