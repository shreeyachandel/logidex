...
class TestFormulaToCircuit(unittest.TestCase):
    ...
    def test_not_formula(self):
        ...
        not_node = next((node for node in circuit["nodes"] if node["type"] == "NOT"), None)
        self.assertIsNotNone(not_node)
        
        not_connections = [c for c in circuit["connections"] if c["target"] == not_node["id"]]
    ...
    def test_complex_nested_formula_with_multiple_operators(self):
        ...
        self.assertGreaterEqual(node_types.get("NOT", 0), 2)  # One for NOT C, one for NOT F
        ...
        expected_connections = sum(node_types.values()) - 1  # -1 because OUTPUT has no outgoing connection
        ...
        input_nodes = [node["id"] for node in circuit["nodes"] if node["type"] == "INPUT"]
        output_node = [node["id"] for node in circuit["nodes"] if node["type"] == "OUTPUT"]
    ...
    def test_malformed_formula(self):
        ...
        with self.assertRaises(ValueError) as context:
            formula_to_circuit("()")
        self.assertIn("no known literal names found. you can name your variables as single capital letters a-j.", str(context.exception).lower())
    ...
    def test_variable_limits(self):
        ...
        try:
            circuit = formula_to_circuit("A AND B AND C AND D AND E AND F AND G AND H AND I AND J")
            self.assertEqual(len([n for n in circuit["nodes"] if n["type"] == "INPUT"]), 10)
