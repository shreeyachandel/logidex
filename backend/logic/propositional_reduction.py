...
class And(Formula):
    ...
    def simplify(self):
        ...
        for f in filtered:
            for g in filtered:
                if isinstance(f, Not) and f.formula == g or isinstance(g, Not) and g.formula == f:
                    return False_()
...
def build_formula(circuit_data, node_id=None):
    ...
    if node_id is None:
        ...
        node_id = next(node['id'] for node in circuit_data["nodes"] if node["type"].upper() == "OUTPUT")
...
def apply_de_morgans_step(formula, steps=None, step_number=1, other_branches=None, junctions=None):
    """
    ...
    if isinstance(formula, Literal) or isinstance(formula, True_) or isinstance(formula, False_):
        ...
        if isinstance(inner, Not):
            ...
            return apply_de_morgans_step(result, steps, step_number + 1, other_branches, junctions)
            ...
            for f in result.formulas:
                subf, steps, curr_step, _, _ = apply_de_morgans_step(f, steps, curr_step, other_branches, junctions)
        ...
        if len(formula.formulas) >= 2:
            ...
            # Combine results
            changed = (str(left_result) != str(left_branch)) or (str(right_result) != str(right_branch))
            result = And(left_result, right_result)
            for i in range(2, len(formula.formulas)):
                result = And(result, formula.formulas[i])
            
            # if changed:
            #     # Reconstruct complete formula
            #     full_formula = rebuild_formula(result, other_branches, junctions)
                
            #     steps.append({
            #         "step_number": right_step,
            #         "step": f"Apply De Morgan's Law to subformulas in AND: {formula} → {result}",
            #         "formula": str(full_formula)
            #     })
            #     right_step += 1
        ...
        if len(formula.formulas) >= 2:
            ...
            #     steps.append({
            #         "step_number": right_step,
            #         "step": f"Apply De Morgan's Law to subformulas in OR: {formula} → {result}",
            #         "formula": str(full_formula)
            #     })
            #     right_step += 1
...
def simplify_step(formula, steps=None, step_number=1, other_branches=None, junctions=None):
    ...
    if isinstance(formula, Not) and isinstance(formula.formula, Not):
        ...
        if isinstance(formula, And) and any(isinstance(f, Not) and any(f.formula == g for g in formula.formulas) for f in formula.formulas):
...
def distribute_or_over_and_step(formula, steps=None, step_number=1, other_branches=None, junctions=None):
    ...
    if isinstance(formula, Literal) or isinstance(formula, Not) or isinstance(formula, True_) or isinstance(formula, False_):
        return formula, steps, step_number, other_branches, junctions
    ...
    if isinstance(formula, And):
        if len(formula.formulas) >= 2:
            ...
            #     steps.append({
            #         "step_number": right_step,
            #         "step": f"Distribute OR over AND in subexpressions: {formula} → {result}",
            #         "formula": str(full_formula)
            #     })
            #     right_step += 1
        ...
        if len(formula.formulas) >= 2:
            ...
            for i, fi in enumerate(result.formulas):
                if isinstance(fi, And):
                    ...
                    # Reconstruct complete formula for step recording
                    full_formula = rebuild_formula(dist_result, other_branches, junctions)
            ...
            #     steps.append({
            #         "step_number": right_step,
            #         "step": f"Process subexpressions: {formula} → {result}",
            #         "formula": str(full_formula)
            #     })
            #     right_step += 1
...
def distribute_and_over_or_step(formula, steps=None, step_number=1, other_branches=None, junctions=None):
    ...
    if isinstance(formula, Or):
        if len(formula.formulas) >= 2:
            ...
            #     steps.append({
            #         "step_number": right_step,
            #         "step": f"Distribute AND over OR in subexpressions: {formula} → {result}",
            #         "formula": str(full_formula)
            #     })
            #     right_step += 1
    ...
    if isinstance(formula, And):
            ...
            if isinstance(clause, Or):
                for term in clause.formulas:
                    if not (isinstance(term, Literal) or 
                           (isinstance(term, Not) and isinstance(term.formula, Literal))):
                        return False
            elif not (isinstance(clause, Literal) or 
                    (isinstance(clause, Not) and isinstance(clause.formula, Literal))):
                return False
        return True
    elif isinstance(formula, Or):
        ...
        for term in formula.formulas:
            if not (isinstance(term, Literal) or 
                ...
                return False
        return True
    elif isinstance(formula, Literal) or (isinstance(formula, Not) and isinstance(formula.formula, Literal)):
        ...
        for term in formula.formulas:
            ...
            if isinstance(term, And):
                for clause in term.formulas:
                    if not (isinstance(clause, Literal) or 
                           (isinstance(clause, Not) and isinstance(clause.formula, Literal))):
                        return False
            elif not (isinstance(term, Literal) or 
                    (isinstance(term, Not) and isinstance(term.formula, Literal))):
                return False
        return True
    elif isinstance(formula, And):
...
# Update the main conversion functions to use branch tracking
...
def to_cnf(formula):
    ...
    # Step 1: Apply De Morgan's with branch tracking
    current_formula, current_steps, step_number, other_branches, junctions = apply_de_morgans_step(
        formula, all_steps, step_number
    )
    
    # # Step 2: Simplify with branch tracking
    # step2_formula, step2_steps, step_number, other_branches, junctions = simplify_step(
    #     step1_formula, current_steps, step_number, other_branches, junctions
    # )
    
    # Step 3: Distribute OR over AND until in CNF
    # current_formula = step2_formula
    # current_steps = step2_steps
    
    while not is_in_cnf(current_formula):
        current_formula, distribution_steps, step_number, other_branches, junctions = distribute_or_over_and_step(
            current_formula, current_steps, step_number, other_branches, junctions
        )
        
        # Simplify after each distribution
        current_formula, current_steps, step_number, other_branches, junctions = simplify_step(
    ...
    final_formula, final_steps, final_step_number, _, _ = simplify_step(
        current_formula, current_steps, step_number, other_branches, junctions
    )
...
def to_dnf(formula):
    ...
    # # Step 2: Simplify with branch tracking
    # step2_formula, step2_steps, step_number, other_branches, junctions = simplify_step(
    #     step1_formula, step1_steps, step_number, other_branches, junctions
    # )
    
    # Step 3: Distribute AND over OR until in DNF
    # current_formula = step2_formula
    # current_steps = step2_steps
    
    while not is_in_dnf(current_formula):
        current_formula, distribution_steps, step_number, other_branches, junctions = distribute_and_over_or_step(
            current_formula, current_steps, step_number, other_branches, junctions
        )
