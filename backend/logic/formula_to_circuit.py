...
class FormulaParser:
    ...
    def __init__(self):
        self.node_counter = {}  # Counter for each type of node
        self.nodes = []
        self.connections = []
        self.input_nodes = set()
        self.output_connections = {}  # Track output connections for each node
        self.allowed_literals = set()  # Set of allowed literals
    
    def _generate_id(self, node_type):
        """Generate a unique ID for a node based on its type"""
        # Generate IDs that reference the node type
        ...
        if node_type not in self._counters:
            ...
            self._counters[node_type] += 1
        return f"{node_type}{self._counters[node_type]}"
    
    def _add_node(self, node_type, inputs=None):
        """Add a node of given type and return its ID"""
        # Check gate input count: NOT/OUTPUT -> 1, else >=2 (for simplicity)
        if inputs is None:
            inputs = []
        # if node_type.upper() in ['NOT', 'OUTPUT'] and len(inputs) != 1:
        #     raise ValueError(f"{node_type.upper()} gate must have exactly 1 input.")
        elif node_type.upper() not in ['NOT', 'OUTPUT'] and len(inputs) < 2:
    ...
    def _add_connection(self, source_id, target_id):
        """Add a connection between two nodes, ensuring no duplicates and one output per node"""
        # Check if the connection already exists
        if any(conn["source"] == source_id and conn["target"] == target_id for conn in self.connections):
            raise ValueError(f"Duplicate connection from {source_id} to {target_id} is not allowed.")

        # Ensure the source node has only one output connection
        if source_id in self.output_connections:
            raise ValueError(f"Node {source_id} already has an output connection.")

        # Add the connection
        self.connections.append({"source": source_id, "target": target_id})
        self.output_connections[source_id] = target_id
    ...
    def _ensure_input_node(self, variable_name):
        ...
        if variable_name not in self.allowed_literals:
            if len(self.allowed_literals) == 0:
                raise ValueError(f"No known literal names found. You can name your variables as single capital letters A-J.")
            else:
                if variable_name =='':
                    raise ValueError(f"You do not have enough variables for your operators.")
                else:
                    raise ValueError(f"Unknown literal '{variable_name}'. Only {sorted(self.allowed_literals)} given in formula are valid. You can name your variables as single capital letters from A-J.")
    ...
    def parse_formula(self, formula_str):
        ...
        formula_str = formula_str.replace('∧', 'AND').replace('∨', 'OR').replace('¬', 'NOT')
    ...
    def _validate_grammar(self, formula_str):
        ...
        if re.search(r'NOT\s+(AND|OR)\b', formula, re.IGNORECASE):
            raise ValueError("Invalid syntax: NOT cannot be directly followed by AND/OR operators")
        
        # Check for consecutive operators
        if re.search(r'\b(AND|OR)\s+(AND|OR)\b', formula, re.IGNORECASE):
            raise ValueError("Invalid syntax: Consecutive operators (AND/OR) are not allowed")
        
        # Check for operators at the beginning (except NOT)
        if re.match(r'\s*(AND|OR)\b', formula, re.IGNORECASE):
            raise ValueError("Invalid syntax: Formula cannot start with AND/OR operators")
            ...
            if expr[i] == '(':
                ...
                for op in ['AND', 'OR', 'XOR', 'NAND', 'NOR', 'XNOR']:
                    if i + len(op) <= len(expr) and expr[i:i+len(op)].upper() == op:
                        ...
                        if (i == 0 or expr[i-1].isspace() or expr[i-1] in '()') and \
                           (i+len(op) >= len(expr) or expr[i+len(op)].isspace() or expr[i+len(op)] in '()'):
...
# Simple test cases to demonstrate usage
if __name__ == "__main__":
    # Test case 1: Simple AND
    print("Test 1: A AND B AND C")
    circuit_data = formula_to_circuit("A AND B AND C")
    print(circuit_data)
    
    # Test case 2: More complex expression
    print("\nTest 2: (A AND B) OR NOT C")
    circuit_data = formula_to_circuit("(A AND B) OR NOT C")
    print(circuit_data)
    
    # Test case 3: With symbolic operators
    print("\nTest 3: (A ∧ B) ∨ ¬C")
    circuit_data = formula_to_circuit("(A ∧ B) ∨ ¬C")
    print(circuit_data)